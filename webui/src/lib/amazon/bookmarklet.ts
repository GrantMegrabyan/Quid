...
export const BOOKMARKLET_SOURCE = `(function(){
  ...
  function valByLabel(card,re){var labels=card.querySelectorAll('.a-text-caps');for(var i=0;i<labels.length;i++){var label=labels[i];if(!re.test(txt(label)))continue;var col=label.closest('.a-column, .a-fixed-right-grid-col')||label.parentElement;if(!col)continue;var row=col.querySelector('.a-row');var v=row?txt(row):txt(col).replace(txt(label),'').replace(/^\\s+/,'').replace(/\\s+$/,'');if(v)return v;}return null;}
  function orderId(card){var byLabel=valByLabel(card,/order\\s*#/i);var fl=byLabel?byLabel.match(/\\d{3}-\\d{7}-\\d{7}/):null;if(fl)return fl[0];for(var i=0;i<IDSEL.length;i++){var v=txt(card.querySelector(IDSEL[i]));var mm=v.match(/\\d{3}-\\d{7}-\\d{7}/);if(mm)return mm[0];}var ft=txt(card).match(/\\d{3}-\\d{7}-\\d{7}/);return ft?ft[0]:null;}
