...
export const BOOKMARKLET_SOURCE = `(function(){
  ...
  var IDSEL=['bdi[dir=ltr]','.yohtmlc-order-id bdi','.yohtmlc-order-id span','[data-testid=order-id]'];
  ...
  function card2order(card,domain,index){var id=orderId(card);if(!id)fail('order card #'+(index+1)+' has no recognisable order id.');var total=money(txt(pick(card,TOTSEL)));if(!total)fail('order '+id+' has no parseable total.');var date=ndate(txt(pick(card,DATESEL)))||'';return {orderId:id,orderDate:date,total:total,currency:null,status:status(card),items:items(card),shipments:[],paymentLast4:last4(card),orderUrl:orderUrl(card,domain)};}
